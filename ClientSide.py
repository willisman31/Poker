...
class PokerClient:
    ...
    def __init__(self, screen):
        ...
        textClient, textClientRect = mygui.print_text('freesansbold.ttf', int(HEIGHT/15), "Client", WHITE, None, int(WIDTH/2), int(HEIGHT/10))
        screen.blit(textClient, textClientRect)

        textIp, textIpRect = mygui.print_text('freesansbold.ttf', int(HEIGHT/30), "Enter IP : ", WHITE, None, int(WIDTH/2-BOXIPWIDTH/2-WIDTH/13),int(HEIGHT/4))
        screen.blit(textIp, textIpRect)

        textPort, textPortRect = mygui.print_text('freesansbold.ttf', int(HEIGHT/30), "Enter Port : ", WHITE, None, int(WIDTH/2-BOXPORTWIDTH/2-WIDTH/13), int(HEIGHT/4+HEIGHT/9.6))
        screen.blit(textPort, textPortRect)

        Button1 = mygui.Button()
        Button1.create_button(screen, BACK_BUTTON, int(WIDTH/2-WIDTH/6), int(HEIGHT/2), int(WIDTH/3), int(WIDTH/9), 0, "Connect", TEXT_BUTTON)

        input_ip = mygui.TextBox((int(WIDTH/2-BOXIPWIDTH/2), int(HEIGHT/4-BOXIPHEIGHT/2), int(BOXIPWIDTH), int(BOXIPHEIGHT)),command=None,clear_on_enter=True,inactive_on_enter=False)
        input_port = mygui.TextBox((int(WIDTH/2-BOXPORTWIDTH/2), int(HEIGHT/4+HEIGHT/10-BOXPORTHEIGHT/2), int(BOXPORTWIDTH), int(BOXPORTHEIGHT)),command=None,
